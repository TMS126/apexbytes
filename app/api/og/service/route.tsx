...
let price: string = ''
let bg: string = `linear-gradient(135deg, ${OG_COLORS.background} 0%, ${OG_COLORS.backgroundMid} 55%, ${OG_COLORS.blue} 100%)`
  if (hubParam && HUB_ORDER.includes(hubParam as HubId) && sectionParam && serviceParam) {
    ...
    if (item) {
      ...
      price = item.price
      const hubColors = HUB_HEX_COLORS[hubParam as HubId]
      bg = `linear-gradient(135deg, ${hubColors.primary} 0%, ${hubColors.tagBgDark} 100%)`
    }
  }
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        {price && (
          <div style={{ display: 'flex', fontSize: '44px', fontWeight: 800, color: OG_COLORS.white, marginTop: '14px' }}>
            {price}
          </div>
        )}
        <div style={{ display: 'flex', fontSize: '28px', fontWeight: 500, color: OG_COLORS.muted, marginTop: '20px' }}>
          {subtitle}
        </div>
      </div>
    ),
